...
kann, Åbernimmt der Autor keinerlei Haftung fÅr unmittelbare oder
FolgeschÑden, die durch den Gebrauch dieses Programms entstehen.

:p.Alle Warenzeichen sind Warenzeichen der jeweiligen Inhaber und werden
ohne RÅcksicht auf freie VerfÅgbarkeit verwendet.
...
Sie kînnen Shareware allerdings vor dem Kauf testen!
...
(komplett, unverÑndert, nicht installiert)
:li.die KopiergebÅhr DM 15,- nicht Åberschreitet
:eul.

:p.Der Autor behÑlt sich rechtliche Schritte bei Verletzung des Copyrights
oder der Lizenzbestimmungen vor.

:p.:lines align=center.:hp7.Registrierung:ehp7.:elines.

:p.Sollten Sie sich nach der Dauer von 30 Tagen dafÅr entscheiden, da·
Sie den CD Spieler weiterhin benutzen mîchten, so sind Sie verpflichtet,
...
in keiner Weise gegenÅber der registrierten Version eingeschrÑnkt.

:p.Die RegistrierungsgebÅhr betrÑgt DM 30,- inkl. Versand und MWSt.
FÅr Zahlungen aus dem Ausland siehe ":link reftype=hd res=19000.REGISTER.TXT:elink.".

:p.Preise fÅr mehr als 2 Lizenzen oder fÅr HÑndler auf Anfrage!

:p.Die Registrierung geschieht durch Einsenden eines Verrechnungsschecks oder von
Bargeld an den Autor (Schecks werden frÅhestens am Versandtag
eingereicht)&colon.
...
:p.Sollten Sie den Betrag lieber Åberweisen wollen&colon.
...
:p.Der Versand kann (in AusnahmefÑllen) bis zu 6 Wochen dauern, bei Scheck-
...
:p.Verwenden Sie bitte das beigefÅgte Formular
":link reftype=hd res=19000.REGISTER.TXT:elink.", das
Sie ausdrucken und verschicken kînnen.
...
:p.Updates sind fÅr registrierte Benutzer bis auf weiteres kostenlos,
wenn das Update via Modem erfolgen kann. Andere registrierte Benutzer
zahlen lediglich die GebÅhr Ihres Shareware-HÑndlers (i.d.R. 5 DM).
...
:p.Um die beiden Seiten auszudrucken, wÑhlen Sie sie aus und benutzen
den Befehl :hp2.Drucken:ehp2. im MenÅ :hp2.Funktionen:ehp2. des Hilfefensters.
:ul.
...
:fig.
    ...
    ____ CD Spieler fÅr OS/2 Registrierung
         Deutschland&colon. DM 30,-  Europa&colon. DM 35,-  Welt&colon. DM 50,-   = DM _____,-

    Spenden ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ> DM _____,-
    ...
    (  ) öberweisung auf das Konto 0313312 der Deutschen Bank Darmstadt
         (BLZ 508 700 05) am __. __. 199__ (bitte Datum eintragen)

    (  ) beiliegendes Bargeld/Scheck Nr. _____________  (Vorsicht Diebstahl)

    Der Versand kann (in AusnahmefÑllen) bis zu 6 Wochen dauern, bei Scheck-
    oder Barzahlung fallen die Banklaufzeiten weg. Aus dem Ausland bitte nur
    D-Mark-Euroschecks oder Bargeld (sonst Bankspesen von ca. 10,- bis 20,-
    hinzufuegen).
...
:fig.
    ...
    Muehe und fÅllen Sie das Formular komplett aus. Vielen Dank!

    Diese CD Spieler Version wurde kopiert&colon.
    ( ) aus einer Mailbox - Name/Tel&colon. _______________________________________
    ( ) von einem Shareware-HÑndler - Name/Ort: _____________________________
    ...
    VerbesserungsvorschlÑge/WÅnsche&colon.
